...
double_pawn_penalty_opening = -10
double_pawn_penalty_endgame = -20

# Isolated pawn penalty
isolated_pawn_penalty_opening = -10
isolated_pawn_penalty_endgame = -20
...
# masking to calculate pawn structure
...
def init_masks():
    ...
    for i_rank, rank in enumerate(RANKS):
        for i_file, file in enumerate(FILES):
            ...
            if i_file == 0:  # A file
                isolated_masks[pointer] = b_file
                white_passed_masks[pointer] = a_file | b_file
                black_passed_masks[pointer] = a_file | b_file
            elif i_file == 7:  # H file
                isolated_masks[pointer] = g_file
                white_passed_masks[pointer] = g_file | h_file
                black_passed_masks[pointer] = g_file | h_file
            else:
...
# game phases
opening, endgame, middle_game = range(3)
